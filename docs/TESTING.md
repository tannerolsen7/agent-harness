...
_None yet — behaviors will be added when the fix ships._

### Known gaps

- **[BUG] `/feature` skill does not create a worktree before implementation begins:**
  The skill only mentions worktrees in a conditional cleanup note after merge. It has
  no setup step that runs `scripts/worktree-add.sh`. As a result, agents using `/feature`
  directly (not via `/queue`) create a `feat/<slug>` branch in the main worktree rather
  than in an isolated worktree at `.claude/worktrees/<slug>`. This contradicts the
  PITFALLS.md rule "Feature work in the main worktree races against background processes"
  and leaves branches without corresponding worktrees.
  Failing test: `tests/feature-worktree-setup.test.sh` — awaiting fix via /feature.

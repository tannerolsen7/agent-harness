...
# coverage line shows n-of-corpus; full coverage (all 5 traps in the run) → no PARTIAL warning
printf '%s' "$out" | grep -q 'of 5 in corpus' && ok || no "--traps should print 'of 5 in corpus' coverage"
printf '%s' "$out" | grep -q 'PARTIAL' && no "--traps full-coverage run must NOT warn PARTIAL" || ok

# --traps honored in ANY arg position (a forgotten-first flag must not silently no-op to green)
out=$(printf '009\tcaught\n012\tmissed\n013\tmissed\n' | bash "$SCORE" /dev/stdin --traps)
tc=$(printf '%s' "$out" | awk '/^trap cases:/{print $3}')
[ "$tc" = "3" ] && ok || no "--traps after the path should still run (trap cases = $tc, want 3)"

# partial coverage (2 of 5 traps in the run) → visible WARN, not a silent shrink
out=$(printf '009\tcaught\n010\tcaught\n' | bash "$SCORE" --traps /dev/stdin)
printf '%s' "$out" | grep -q 'WARN:.*PARTIAL' && ok || no "--traps partial run should WARN PARTIAL coverage"

# --traps but NO trap cases in the run → guard measured nothing → exit 3 (must not read as pass)
out=$(printf '001\tcaught\n002\tmissed\n' | bash "$SCORE" --traps /dev/stdin); rc=$?
[ "$rc" -eq 3 ] && ok || no "--traps with no trap rows should exit 3, got $rc"
printf '%s' "$out" | grep -q 'measured nothing' && ok || no "--traps empty subset should print 'measured nothing'"

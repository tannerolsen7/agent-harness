...
- A deleted category-1 file is re-created on sync; a deleted category-2 file is also re-created
  (the template, not the project's customized content — but since the file was deleted, the
  template is the only thing that can be restored)
...
mixed in with the harness-owned config. These must be separated.
...
- `.claude/settings.local.json` is for project-specific **permission customizations only** (e.g.
  allowing specific bash commands). Not required on day one. If needed, install.sh creates a
  minimal template. Category-3: sync never touches it.
...
walks the user through filling it in.

The `autoMode.environment` block is removed from the harness entirely. Project context goes in
CLAUDE.md. Permission customizations go in `settings.local.json`.
...
- Copies `CLAUDE.md`, `AGENTS.md`, `CONTEXT.md`, `PITFALLS.md` from `docs/templates/` for any
  that are missing
- For `CLAUDE.md`: walks the user through filling in each section — project name, tech stack,
  package manager, source control host, deployment target, and anything else Claude should know
- Reports what it created vs. what it skipped (already existed)
- Does not re-run install.sh

**The setup checklist in the CLAUDE.md template**

`docs/templates/CLAUDE.md` ships with a `## Setup checklist` section at the very top. It is
the first thing a newly-installed user sees when they open CLAUDE.md:
...
The checklist lives in CLAUDE.md — not only in terminal output from install.sh — because that
is where the user will be when they need it. Once the three steps are done, they delete this
section.
...
4. **settings.local.json in git** — committed when needed (permission customizations only). Most
   projects will not need it on day one. Project context lives in CLAUDE.md instead.

5. **Husky wiring** — separate `scripts/install-harness-hooks.sh`. install.sh prints the command
   to run; the user inspects and runs it themselves.

6. **Template scope** — templates ship in `docs/templates/`. The `/init` skill copies them and
   walks the user through filling in CLAUDE.md interactively. Project context goes in CLAUDE.md,

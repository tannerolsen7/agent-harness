...
(cd "$TMP1" && unset GIT_DIR GIT_WORK_TREE && bash "$SCRIPT" "$TMP1_WT" "feat/alpha-task") >/dev/null 2>&1
...
(cd "$TMP2" && unset GIT_DIR GIT_WORK_TREE && bash "$SCRIPT" "$TMP2_WT" "chore/something") >/dev/null 2>&1
...
(cd "$TMP3" && unset GIT_DIR GIT_WORK_TREE && bash "$SCRIPT" "$TMP3_WT" "feat/no-tasks-file") >/dev/null 2>&1
